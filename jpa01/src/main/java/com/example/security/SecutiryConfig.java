...
public class SecutiryConfig extends WebSecurityConfigurerAdapter{
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		//로그인 설정
		.formLogin()
		.loginPage("/user/login") //127.0.0.1:9090/ROOT/login
		.loginProcessingUrl("/user/loginProcess") //<form action="loginProcess" />
		.usernameParameter("userid") //<input type="text" name="userid" />
		.passwordParameter("userpw") //<input type"password" name="userpw />
		.permitAll()
		.defaultSuccessUrl("/user/home") // 로그인 성공시 127.0.0.1:9090/ROOT/customer/home 으로 이동
		.and()
